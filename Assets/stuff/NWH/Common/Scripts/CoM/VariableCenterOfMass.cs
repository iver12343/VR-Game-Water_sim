...
namespace NWH.Common.CoM
{
    ...
    public class VariableCenterOfMass : MonoBehaviour
    {
        ...
        public void UpdateAllProperties(bool applyUnchanged = false)
        {
            if (!useDefaultMass) UpdateMass(applyUnchanged);
            if (!useDefaultCenterOfMass) UpdateCoM(applyUnchanged);
            if (!useDefaultInertia) UpdateInertia(applyUnchanged);
        }

        public void UpdateMass(bool applyUnchanged = false)
        {
            float prevMass = combinedMass;
            ...
            if (prevMass != combinedMass || applyUnchanged)
            {
        ...
        /// </summary>
        public void UpdateCoM(bool applyUnchanged = false)
        {
            Vector3 prevCoM = combinedCenterOfMass;
            ...
            if (prevCoM != combinedCenterOfMass || applyUnchanged)
            {
        ...
        {
            Vector3 prevInertiaTensor = combinedInertiaTensor;
            ...
            if (prevInertiaTensor != inertiaTensor || applyUnchanged)
            {
...
namespace NWH.Common.CoM
{
    ...
    public class VariableCenterOfMassEditor : NUIEditor
    {
        public override bool OnInspectorNUI()
        {
            ...
            Rigidbody parentRigidbody = vcom.GetComponentInParent<Rigidbody>();
            ...
            if (!drawer.Field("useDefaultCenterOfMass").boolValue)
            {
                ...
                if (vcom.useMassAffectors)
                {
                    drawer.Field("centerOfMassOffset", false);
